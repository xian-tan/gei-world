...
- 当前测试基线：`87 passed`。
- 已开始多人联机前置改造：保留默认轮流回合玩法，并新增同时回合制核心、UI 本地玩家视角、会话抽象、网络 DTO、本机服务端权威原型、本地双客户端调试入口、进程内多人房间 UI 原型、HTTP + 轮询传输层、HTTP 网络会话和 HTTP 多人 UI 接入。
